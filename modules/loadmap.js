...
define(function (require, exports, module) {
    ...
    // 获取整栋楼的空车位数据
    YFMap.prototype.getSpaceUnitListOfRegion = function queryAppRegionList(callback) {
        var url = 'http://wx.indoorun.com/chene/getSpaceUnitListOfRegion';
        jsLib.ajax({
            type: "get",
            dataType: 'jsonp',
            url: url, //添加自己的接口链接
                ...
                success: function (str) {
                    ...
                    if (data != null) {
                        if (data.code == "success") {
                            ...
                            // 显示比列尺
                            if (gV.bShowRule) {
                                YFMap.prototype.comparingRule();
                            };

                            oMapUtils.initIdrRouter();
    ...
    YFMap.prototype.comparingRule = function() {

        oMapUtils.comparingRule();
        oMapUtils.calculationRule(100 * gV.scale);
    };

    YFMap.prototype.notShowRule = function() {
        oMapUtils.notShowRule();
    };

    YFMap.prototype.TwoPointDistance = function(p1, p2, bool) {
        return oMapUtils.idrRouteResult(p1, p2, bool);
    };
